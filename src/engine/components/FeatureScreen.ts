...
export class FeatureScreen extends Container {
    ...
    private setupPreview(): void {
        ...
        this._previewText = new Text({ text: this.setPreviewText(this._buttonIndex), style: GameConfig.style.clone() });
    ...
    private setupPreviewTransition(): void {
        ...
        for (let index = 0; index < 3; index++) {
            ...
            buttonFrame.on('pointerover', () => {
                ...
                gsap.to(buttonFrame, { angle: 360, duration: 2, ease: 'none', repeat: -1 });
            });
    ...
    private setupPreviewSymbols(): Container {
        ...
        const symbolNames = Array.from({ length: grid.length }, () => `Symbol${Math.floor(Math.random() * 11) + 1}`);
    ...
    private setupSpinButton(): void {
        ...
        spinButtonFrame.on('pointerout', () => {
            gsap.to(spinButtonIcon, { rotation: 0, duration: 0.5, ease: 'power1.out' });
        });
    ...
    private setButtonIndex(index: number) {
        ...
        gsap.to(this._previewButton.position, { x: -85 + index * 85, duration: 0.4, ease: 'power1.out' });
        gsap.to(this._previewButton.scale, {
            x: 0.5, y: 0.5, duration: 0.2, ease: 'power1.out', onComplete: () => {
                gsap.to(this._previewButton.scale, { x: 1, y: 1, duration: 0.2, ease: 'power1.out' });
            }
        });

        this._previewElements[this._buttonIndex].forEach(element => {
            gsap.killTweensOf(element);
            gsap.fromTo(element, { alpha: 1 }, { alpha: 0, duration: 0.4, ease: 'none', onComplete: () => { element.visible = false } });
        });
        this._previewElements[index].forEach(element => {
            gsap.killTweensOf(element);
            gsap.fromTo(element, { alpha: 0 }, { alpha: 1, duration: 0.4, ease: 'none', onStart: () => { element.visible = true } });
        });
