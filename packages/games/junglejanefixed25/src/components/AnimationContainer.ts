...
export class AnimationContainer extends Container {
    ...
    private constructor(app: Application) {
        ...
        this._winLines.setAvailableLines(25);
    ...
    private eventListeners(): void {
        signals.on(SIGNAL_EVENTS.WIN_ANIMATION_TOTAL_PLAY, async (totalWinAmount: number | undefined) => {
            if (totalWinAmount !== undefined) {
                await this.playTotalWinAnimation(totalWinAmount);

                this._winLines.hideAllLines();
                signals.emit(SIGNAL_EVENTS.WIN_ANIMATION_TOTAL_PLAY_COMPLETE);
            }
        });

        signals.on(SIGNAL_EVENTS.WIN_ANIMATION_SHOW_ALL_LINES, (lines: number[] | undefined) => {
            if (lines !== undefined) {
