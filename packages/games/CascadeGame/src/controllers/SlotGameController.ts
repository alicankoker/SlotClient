...
import {
    ...
    debug,
    SIGNAL_EVENTS,
    CascadeSpinContainer,
    CascadeSpinController,
    IResponseDataNew
...
export class SlotGameController implements ISlotGameController {
    ...
    public initialize(): void {
        ...
        this.spinContainer = new CascadeSpinContainer(this.app, spinContainerConfig) as CascadeSpinContainer;
        ...
        this.spinController = new CascadeSpinController(this.spinContainer as CascadeSpinContainer, {
    ...
    public async executeGameSpin(action: IPayload["action"]): Promise<void | boolean> {
        const balance: number = 10000; //GameDataManager.getInstance().getResponseData()?.balance.after ?? GameDataManager.getInstance().getInitialData()!.balance ?? 0;
        ...
        if ((balance - bet) < 0) {
            ...
            const response: IResponseDataNew = await this.gameServer.processRequest(action);
    ...
    private async onAllReelsStopped(): Promise<void> {
        ...
        Bonus.instance().isActive = (response as IResponseDataNew).bonus !== undefined ? true : false;
        ...
        if (this.gameConfig.WIN_EVENT.enabled && (response as IResponseDataNew).winEventType !== 'normal') {
            const winAmount = (response as IResponseDataNew).totalWin;
            const backendType = (response as IResponseDataNew).winEventType;
        ...
        eventBus.emit("setBalance", (response as IResponseDataNew).balance.after);
    ...
    private async playScatterHighlightAnimation(): Promise<void> {
        if (GameDataManager.getInstance().getResponseData() === undefined || (GameDataManager.getInstance().getResponseData() as IResponseDataNew).steps[GameDataManager.getInstance().getCurrentStepId()].reelsBefore  === undefined) {
            return;
        }

        const reels: number[][] = (GameDataManager.getInstance().getResponseData() as IResponseDataNew).steps[GameDataManager.getInstance().getCurrentStepId()].reelsBefore;
