...
import {
  ...
  StepData,
  SymbolData,
} from "@slotclient/engine/types/ICommunication";
...
export class GameServer {
  ...
  public async processRequest(action: IPayload["action"]): Promise<any> {
    const payload: IPayload = { action: action, data: { lines: GameDataManager.getInstance().getCurrentLine(), betIndex: GameDataManager.getInstance().getBetValueIndex(), bonus: true } }
